...
After developing your method you can add it to the nextflow workflow that can be found at `task_name/workflows/run/main.nf`
...
`viash ns build -q common|task_name --paralell --setup cachedbuild`
...
There is a general unit test that you can find in the `comp_method.yaml` that will be done when executing the command. if you added a specific unit test for your method it will also be run if added correctly.

whenever you change your config file you need to perform a `viash run path/to/method/config.vsh.yaml -- ---setup cachedbuild`
...
As a final note when adding a new method add yourself to the `authors.yaml` file.
